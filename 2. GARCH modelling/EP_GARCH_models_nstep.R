...
vec_msgarch_std_1 = vec_msgarch_std_1[(diff+1):length(vec_msgarch_std_1)]
vec_msgarch_ged_1 = vec_msgarch_ged_1[(diff+1):length(vec_msgarch_ged_1)]
...
#Export intermediate csv's
write.table(vec_msgarch_norm_12, "./vec_msgarch_norm_12_intmed.csv", sep = ",", row.names = FALSE, col.names = TRUE, quote = TRUE)
write.table(vec_msgarch_std_12, "./vec_msgarch_std_12_intmed.csv", sep = ",", row.names = FALSE, col.names = TRUE, quote = TRUE)
write.table(vec_msgarch_ged_12, "./vec_msgarch_ged_12_intmed.csv", sep = ",", row.names = FALSE, col.names = TRUE, quote = TRUE)
write.table(vec_msgarch_norm_24, "./vec_msgarch_norm_24_intmed.csv", sep = ",", row.names = FALSE, col.names = TRUE, quote = TRUE)
write.table(vec_msgarch_std_24, "./vec_msgarch_std_24_intmed.csv", sep = ",", row.names = FALSE, col.names = TRUE, quote = TRUE)
write.table(vec_msgarch_ged_24, "./vec_msgarch_ged_24_intmed.csv", sep = ",", row.names = FALSE, col.names = TRUE, quote = TRUE)
...
fit = ugarchfit(spec, data = log_returns[10000:10500], 
                solver.control = list(solver = "nlminb"))
...
# Assuming ts_data is your time series
ts_data <- log_returns_train
df <- data.frame(y = as.numeric(ts_data), x = 1:length(ts_data))

# The formula indicates that y is the response variable and x is the explanatory variable.
bp <- breakpoints(y ~ x, data = df)
...
# Assuming you have a time series data
# ts_data is your time series object
# "frequency" is the number of observations per year. For example, for monthly data, frequency=12
ts_data <- ts(log_returns_train, frequency = 24)
decomposed_data <- stl(ts_data, s.window="periodic", robust = TRUE)
