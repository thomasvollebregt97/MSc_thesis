forecast_eval = function(data, model, distr, h, refit_every){
  ...
  roll = ugarchroll(spec, log_returns, n.ahead = 1, 
                    n.start = n_start, refit.every = refit_every) #Window size seems equal to starting point
  
  # Roll GARCH model forward in time to produce forecast & extract forecasted volatility/std deviation
